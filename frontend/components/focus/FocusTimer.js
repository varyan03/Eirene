...
import { Play, Pause, RefreshCw, CheckCircle, ArrowLeft } from 'lucide-react';

export default function FocusTimer({ task, onComplete, onBack }) {
    // Determine target duration from the task object (passed by FocusTimeline)
    const durationMinutes = task.targetDuration || 25;
    const FOCUS_TIME = durationMinutes * 60;
    ...
    useEffect(() => {
        if (timeLeft === 0 && isActive) {
            ...
            setIsActive(false);
            // eslint-disable-next-line react-hooks/set-state-in-effect
            setIsFinished(true); // Trigger positive closure
        }
    }, [timeLeft, isActive]);
    ...
    return (
        <div className="bg-white/30 backdrop-blur-md rounded-3xl p-8 shadow-lg border border-white/50 w-full max-w-lg mx-auto flex flex-col items-center animate-fadeIn">
            ...
            <h2 className="text-xl font-semibold text-[#636e72] mb-1">Current Focus</h2>
            <h3 className="text-2xl font-bold text-[#2d3436] mb-8 text-center">{task.text}</h3>

            <div className="relative mb-8">
                {/* Pulse Effect Background */}
                {isActive && (
                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-[#a29bfe]/20 rounded-full animate-ping opacity-75"></div>
                )}
                ...
                {/* Time Display */}
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-5xl font-bold text-[#2d3436] font-mono tracking-wider z-20">
                    {formatTime(timeLeft)}
                </div>
            </div>
            ...
            <div className="flex gap-6 relative z-20 mb-6">
                <button
                    onClick={toggleTimer}
                    className="w-16 h-16 rounded-full bg-gradient-to-r from-orange-400 to-rose-400 text-white flex items-center justify-center shadow-lg transform hover:scale-105 active:scale-95 transition-all"
                >
            ...
            <button
                onClick={stopEarly}
                className="text-sm font-medium text-[#636e72] hover:text-[#2d3436] flex items-center gap-2 transition-colors relative z-20 hover:underline"
            >
